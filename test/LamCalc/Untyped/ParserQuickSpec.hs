...
instance Pretty Expr where
  pretty =
    \case
      ...
      Lam varNames e' ->
        pretty 'λ' <> hsep (pretty <$> varNames) <> pretty '.' <> prettyR e'
...
instance Arbitrary Expr where
  arbitrary = sized tree
    where
      tree 0 = Var <$> varName
      tree n =
        oneof [liftM2 Lam (listOf1 varName) subtree, liftM2 App subtree subtree]
        where
  ...
  shrink =
    para $ \case
      ...
      AppF (lhs, altsL) (rhs, altsR) ->
        (App lhs <$> altsR) <> ((`App` rhs) <$> altsL) <>
        [lhs, rhs, Var "__shrunkApp"]

varName :: Gen Text
varName = do
  head <- frequency [(4, charThat isLower), (1, pure '_')]
  body <-
    resize 3 $ listOf $ frequency [(4, charThat isAlphaNum), (1, pure '_')]
...
spec =
  describe "expr" $
  it "parses arbitrary pretty-printed expressions" $
  property $ \x -> (sut . serialize) x == Just x
